...
def plot_p_cartesian(
    ...
    for ax, p_target, p3_recons, bins, name in zip(
        axs, p3_targets, p3_recons, ranges, labels
    ):
        if not fill:
            ax.hist(
                p3_recons.flatten(),
    ...
    fig.suptitle(
        rf"Distribution of target and reconstructed particle $p_x$, $p_y$, and $p_z$ of {jet_name} jets",
        y=1.03,
    )
...
def plot_p_polar(
    ...
    for ax, p_target, p3_recons, bins, name in zip(
        axs, p3_polar_target, p3_polar_recons, ranges, labels
    ):
        if not fill:
            ax.hist(
                p3_recons.flatten(),
    ...
    if abs_coord:
        fig.suptitle(
            r"Distribution of target and reconstructed particle $p_\mathrm{T}$, $\eta$, and $\phi$ "
            + f"of {jet_name} jets",
            y=1.03,
        )
    else:
        fig.suptitle(
            r"Distribution of target and reconstructed particle $p_\mathrm{T}^\mathrm{rel}$, $\eta^\mathrm{rel}$, and $\phi^\mathrm{rel}$ "
            + f"of {jet_name} jets",
            y=1.03,
        )
