...
def train(
    ...
    for i, data in enumerate(tqdm(loader)):
        ...
        if is_train:
            ...
            if ("emd" in args.loss_choice.lower()) and (
                (i % args.save_freq) == 0 and i > 0
            ):
                torch.save(
                    encoder.state_dict(),
...
def train_loop(
    ...
    total_epoch = (
        args.num_epochs if not args.load_to_train else args.num_epochs + args.load_epoch
    )
    ...
    for ep in range(1, args.num_epochs + 1):
        ...
        valid_avg_loss, valid_recons, valid_target, valid_latent = validate(
            args, valid_loader, encoder, decoder, epoch, outpath, device=device
        )
        ...
        if args.plot_freq > 0:
            if epoch >= args.plot_start_epoch:
                plot_epoch = ((epoch + 1) % args.plot_freq == 0) or (
                    num_stale_epochs == 0
                )
            else:
                plot_epoch = False
        ...
        if args.plot_freq > 0:
            if (epoch > 0) and (epoch % int(args.plot_freq) == 0):
                plot_eval_results(
                    args,
                    data=(train_avg_losses, valid_avg_losses),
                    data_name="Losses",
                    outpath=outpath,
                    start=epoch - args.plot_freq,
                )
...
def get_loss(
    ...
    if args.loss_choice.lower() in ["chamfer", "chamferloss", "chamfer_loss"]:
        ...
        batch_loss = chamferloss(
            p_recons, p_target, jet_features_weight=args.chamfer_jet_features_weight
        )  # output, target
