...
class PermutationTest:
    ...
    def __call__(
        ...
        if isinstance(x, DataLoader):
            ...
            raise TypeError("x must be a DataLoader or a Tensor. " f"Found: {type(x)}")
        ...
        if save_dir is not None:
            ...
            path_eqv = None
        return {
            "invariance": get_dev_summary(
                inv_dev, perm=perm_inv, verbose=verbose, save_path=path_inv
            ),
            "equivariance": get_dev_summary(
                eqv_dev, perm=perm_eqv, verbose=verbose, save_path=path_eqv
            ),
        }
...
def dev(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Deviation of output from target."""
    return (output - target).abs() / (target.abs() + EPS)


def apply_perm(perm: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
...
def get_model_output(
    x: torch.Tensor, encoder: Encoder, decoder: Decoder
) -> torch.Tensor:
