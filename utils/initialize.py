...
def initialize_models(args):
    ...
    logging.info(
        f"Total parameters: {encoder.num_learnable_params + decoder.num_learnable_params}"
    )
    ...
    if args.load_to_train:
        ...
        try:
            if (
                ...
                model_path
                / f"weights_encoder/epoch_{args.load_epoch-1}_encoder_weights.pth"
            ).exists():
                logging.warning(
                    f"No model at epoch {args.load_epoch} found in {model_path}. Searching for epoch {args.load_epoch - 1}."
                )
                ...
                _load_weights(
                    decoder,
                    model_path
                    / f"weights_decoder/epoch_{args.load_epoch-1}_decoder_weights.pth",
                    map_location=args.device,
                )
                ...
                logging.warning(
                    f"No model at epoch {args.load_epoch - 1} found in {model_path}. Searching for best epoch."
                )
                logging.warning(
                    f"Epoch {args.load_epoch} Not found. Loading the best model instead of the specified epoch."
                )
                _load_weights(
                    encoder,
                ...
                logging.warning(
                    f"No model at best epoch found in {model_path} Training from scratch."
                )
                epoch = "None"

        except FileNotFoundError:
            logging.warning(
                f"No model at epoch {args.load_epoch} found in {model_path}. Training from scratch."
            )
...
def _load_weights(
    model: torch.nn.Module, path: Path, model_name: str = None, *args, **kwargs
) -> None:
...
def initialize_optimizers(
    args: Namespace, encoder: Encoder, decoder: Decoder
    ...
    if args.optimizer.lower() == "adam":
        ...
        optimizer_encoder = torch.optim.RMSprop(
            encoder.parameters(), lr=args.lr, eps=1e-16, momentum=0.9
        )
        optimizer_decoder = torch.optim.RMSprop(
            decoder.parameters(), lr=args.lr, eps=1e-16, momentum=0.9
        )
    elif args.optimizer.lower() == "adagrad":
        optimizer_encoder = torch.optim.Adagrad(
            encoder.parameters(), lr=args.lr, eps=1e-16
        )
        optimizer_decoder = torch.optim.Adagrad(
            decoder.parameters(), lr=args.lr, eps=1e-16
        )
    elif args.optimizer.lower() == "sgd":
        optimizer_encoder = torch.optim.SGD(
            encoder.parameters(), lr=args.lr, momentum=0.9
        )
        optimizer_decoder = torch.optim.SGD(
            decoder.parameters(), lr=args.lr, momentum=0.9
        )
    else:
...
def initialize_test_dataloader(
    paths: Union[List[Path], List[str], Path, str], batch_size: int, vec_dims: int = 3
) -> DataLoader:
    logging.info(f"Loading from {paths}")
