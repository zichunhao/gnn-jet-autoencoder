...
class Decoder(nn.Module):
    def __init__(
        ...
        if self.latent_map.lower().replace(" ", "_") in LOCAL_MIX:
            ...
            self.linear = (
                nn.Linear(latent_node_size, self.node_sizes[0][0])
                .to(self.device)
                .to(self.dtype)
            )
        else:
            self.linear = (
                nn.Linear(latent_node_size, self.num_nodes * self.node_sizes[0][0])
                .to(self.device)
                .to(self.dtype)
            )
    ...
    def forward(self, x: torch.Tensor, metric="euclidean") -> torch.Tensor:
