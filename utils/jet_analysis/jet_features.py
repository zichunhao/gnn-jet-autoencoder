...
LABELS_CARTESIAN_ABS_COORD = (
    r'$M$ (GeV)', r'$P_x$ (GeV)', r'$P_y$ (GeV)', r'$P_z$ (GeV)')
LABELS_CARTESIAN_REL_COORD = (
    r'$M^\mathrm{rel}$', r'$P_x^\mathrm{rel}$', r'$P_y^\mathrm{rel}$', r'$P_z^\mathrm{rel}$')
LABELS_POLAR_ABS_COORD = (
    r'$M$ (GeV)', r'$P_\mathrm{T}$ (GeV)', r'$\eta$', r'$\phi$')
LABELS_POLAR_REL_COORD = (
    r'$M^\mathrm{rel}$', r'$P_\mathrm{T}^\mathrm{rel}$', r'$\eta^\mathrm{rel}$', r'$\phi^\mathrm{rel}$')
...
def plot_jet_p_cartesian(
    ...
    for ax, p_target, p_recons, bins, name in zip(axs, jet_features_target, jet_features_recons, ranges, names):
        if not fill:
            ...
            ax.hist(p_recons.flatten(), bins=bins, alpha=0.6,
                    label='reconstructed', density=density)
            ax.hist(p_target.flatten(), bins=bins, alpha=0.6,
                    label='target', density=density)
    ...
    if abs_coord:
        fig.suptitle(
            fr'Distribution of target and reconstructed jet $M$, $P_x$, $P_y$, and $P_z$ of {jet_name} jet', y=1.03)
    else:
...
def plot_jet_p_polar(
    ...
    for ax, p_target, p_recons, bins, name in zip(axs, jet_features_target, jet_features_recons, ranges, names):
        if not fill:
            ...
            ax.hist(p_recons.flatten(), bins=range, alpha=0.6,
                    label='reconstructed', density=density)
            ax.hist(p_target.flatten(), bins=range, alpha=0.6,
                    label='target', density=density)
