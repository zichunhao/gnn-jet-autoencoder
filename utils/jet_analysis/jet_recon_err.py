...
def get_bins(
    ...
    if rel_err_cartesian is None:
        ...
        mass_min_max = (
            -min(10 * np.std(rel_err_cartesian[0]), 1),
            min(2 * np.std(rel_err_cartesian[0]), 2 * MAX_BIN_RANGE),
        )
        px_min_max = (
            -min(np.std(rel_err_cartesian[1]), MAX_BIN_RANGE),
            min(np.std(rel_err_cartesian[1]), MAX_BIN_RANGE),
        )
        py_min_max = (
            -min(np.std(rel_err_cartesian[2]), MAX_BIN_RANGE),
            min(np.std(rel_err_cartesian[2]), MAX_BIN_RANGE),
        )
        pz_min_max = (
            -min(np.std(rel_err_cartesian[3]), MAX_BIN_RANGE),
            min(np.std(rel_err_cartesian[3]), MAX_BIN_RANGE),
        )
    ...
    if rel_err_polar is None:
        ...
        mass_min_max = (
            -min(10 * np.std(rel_err_polar[0]), 1),
            min(2 * np.std(rel_err_polar[0]), 2 * MAX_BIN_RANGE),
        )
        pt_min_max = (
            -min(10 * np.std(rel_err_polar[1]), 1),
            min(2 * np.std(rel_err_polar[1]), 2 * MAX_BIN_RANGE),
        )
        eta_min_max = (
            -min(np.std(rel_err_polar[2]), MAX_BIN_RANGE),
            min(np.std(rel_err_polar[2]), MAX_BIN_RANGE),
        )
        phi_min_max = (
            -min(np.std(rel_err_polar[3]), MAX_BIN_RANGE),
            min(np.std(rel_err_polar[3]), MAX_BIN_RANGE),
        )
