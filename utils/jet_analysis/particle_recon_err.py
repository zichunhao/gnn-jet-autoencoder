...
def plot_particle_recon_err(
    ...
    if not find_match:
        rel_err_cartesian = get_rel_err(p_target_cartesian, p_recons_cartesian).view(
            -1, 3
        )
    ...
    for rel_err, p_padded_recons, coordinate, bin_tuple, labels in zip(
        ...
        if save_dir:
            if epoch is not None:
                ...
                plt.savefig(
                    osp.join(
                        path, f"particle_reconstruction_errors_epoch_{epoch+1}.pdf"
                    )
                )
                ...
                plt.savefig(
                    osp.join(
                        save_dir, f"particle_reconstruction_errors_{coordinate}.pdf"
                    )
                )
...
def get_rel_err_find_match(
    ...
    for i in range(len(p_target_cartesian)):
        ...
        rel_err_cartesian = (
            p_recons_cartesian[i][matching[1]] - p_target_cartesian[i]
        ) / p_target_cartesian[i]
        rel_err_cartesian_list.append(rel_err_cartesian)

        rel_err_polar = (
            p_recons_polar[i][matching[1]] - p_target_polar[i]
        ) / p_target_polar[i]
...
def get_min_max(err: np.ndarray, alpha: float = 4) -> Tuple[Tuple[float, float], ...]:
...
def get_bins(
    ...
    ranges_polar_real = tuple(
        [np.linspace(*polar_real_min_max[i]) for i in range(len(polar_real_min_max))]
    )
